...
9. Set the flag `cs_equal_one`. The computation of Omega_GW(k) can be done for a universe behaving like a perfect adiabatic fluid, or a universe whose energy is dominated by a canonical scalar field. In the former case the propagation speed of scalar fluctuations c_s is related to the equation of state parameter as c_s^2=w, while in the latter case c_s^2=1. See the documentation file "SIGWfastGuide.pdf" for more details. By setting `cs_equal_one = True` the computation is performed for the canonical scalar field case, while for `cs_equal_one = False` it is the adiabatic perfect fluid result that is computed. In the latter case the value of `w` in the previous step has to be chosen in the range 0 < `w` < 1.
